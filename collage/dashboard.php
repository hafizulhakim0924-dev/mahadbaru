...
<body>
    <div class="container">
        <div class="sidebar">
            ...
            <div class="user-info">
                <h3><?= htmlspecialchars($student['name'] ?? 'Siswa') ?></h3>
                <p>ID: <?= htmlspecialchars($student['id'] ?? 'N/A') ?></p>
                <p><?= htmlspecialchars($student['class'] ?? 'Kelas') ?></p>
            </div>
            ...
            <div class="menu-item" onclick="showPage('absensi')">
                <i>✅</i> Absensi
            </div>
            <div class="menu-item" onclick="showPage('belanja')">
                <i>🛒</i> Belanja
            </div>
        </div>
...
<div id="pembayaran" class="page">
    ...
    <div class="card" style="padding:0; overflow:hidden;">
        ...
        ></iframe>
            ...
            <div id="absensi" class="page">
                ...
                <div class="card">
                    ...
                    <?php else: ?>
                        ...
                        ?>
                        <div class="total-sks" style="margin-top: 20px;">
                            <strong>Statistik Kehadiran:</strong><br>
                            Total: <?= $total ?> | Hadir: <?= $hadir ?> | Izin: <?= $izin ?> | Sakit: <?= $sakit ?> | Alpha: <?= $alpha ?><br>
                            <strong>Persentase Kehadiran: <?= $persentase ?>%</strong>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <!-- Belanja -->
            <div id="belanja" class="page">
                <div class="header">
                    <h1>Belanja Kebutuhan Sekolah</h1>
                    <button class="logout-btn">Logout</button>
                </div>
                    ...
                    <?php else: ?>
                        <div class="jadwal-grid">
                            <?php foreach ($barang_list as $barang): ?>
                                <div class="jadwal-card" style="cursor: pointer;" onclick="addToCart(<?= $barang['id'] ?>, '<?= htmlspecialchars($barang['nama_barang'], ENT_QUOTES) ?>', <?= $barang['harga'] ?>, <?= $barang['stok'] ?>)">
                                    <h4><?= htmlspecialchars($barang['nama_barang']) ?></h4>
                                    <p><?= htmlspecialchars($barang['deskripsi'] ?? '') ?></p>
                                    <p><strong>Harga: Rp <?= number_format($barang['harga'], 0, ',', '.') ?></strong></p>
                                    <p>Stok: <?= $barang['stok'] ?> unit</p>
                                    <button class="btn btn-primary btn-small" style="margin-top: 10px;">Tambah ke Keranjang</button>
                                </div>
                            <?php endforeach; ?>
                        </div>
                        ...
                        <div id="cart-section" style="margin-top: 30px; display: none;">
                            <div class="card">
                                ...
                                <div id="cart-total" class="total-sks" style="margin-top: 20px;"></div>
                ...
                <div class="card" style="margin-top: 20px;">
                    <h2>Voucher Pembayaran Saya</h2>
                    <?php
                    // Get vouchers for current student
                    $stmt = $conn->prepare("
                        SELECT v.*, p.total_harga, p.status as status_pesanan
                    ...
                    ?>
                        <p style="text-align: center; color: #666; padding: 40px;">
                            Anda belum memiliki voucher pembayaran.
                        </p>
                    <?php else: ?>
                        <table>
                            <thead>
                            ...
                            <tbody>
                                <?php foreach ($vouchers as $voucher): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge <?= $voucher['status_pesanan'] == 'berhasil' ? 'badge-success' : 'badge-warning' ?>">
                        ...
                        <p style="margin-top: 15px; padding: 15px; background: #fff3cd; border-radius: 8px; color: #856404;">
                            <strong>Catatan:</strong> Setelah pembayaran berhasil, tunjukkan voucher ini ke admin kampus untuk menukarkan barang yang dibeli.
                        </p>
                    <?php endif; ?>
                </div>
            </div>
    ...
    <script>
        ...
        function updateCartDisplay() {
            ...
            let html = '<table><thead><tr><th>Barang</th><th>Harga</th><th>Jumlah</th><th>Subtotal</th><th>Aksi</th></tr></thead><tbody>';
            let total = 0;
            ...
            cart.forEach(item => {
                ...
                html += `
                    <tr>
                        ...
                        <td><button class="btn btn-danger btn-small" onclick="removeFromCart(${item.id})">Hapus</button></td>
                    </tr>
                `;
            });
